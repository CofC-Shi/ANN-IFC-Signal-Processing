...
def bigaus_beads(bead_diameters, event_locs, data_time, num_points):
    ...
    for diameter in bead_diameters:
        ...
        params[diameter]['pos_amp_mean'] = -(diameter / 9.3e-5) ** 3
    ...
    for i in range(num_events):
        ...
        a_1 = np.random.normal(params[diameter]['neg_amp_mean'],
                               params[diameter]['neg_amp_sd'])  # Negative Peak Amplitude
        a_2 = np.random.normal(params[diameter]['pos_amp_mean'],
                               params[diameter]['pos_amp_sd'])  # Positive Peak Amplitude
...
def create_data_folder(folder_name, params):
    ...
    os.mkdir(f'{folder_name}/noisy')
    os.mkdir(f'{folder_name}/clean')
...
bead_diameters = [7.32e-6, 7.32e-6]  # Provide two sizes, even if they are the same.
...
folder = f'data/noiselevel7e-4_30streams_symmetric'
